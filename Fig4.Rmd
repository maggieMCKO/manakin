---
title: "Manakin figure 4 related"
author: "Maggie"
date: "2026-02-28"
output:
  github_document:
...
This [R Markdown](http://rmarkdown.rstudio.com) Notebook contains codes for reproducing Fig.4A, 4B and Fig.S4A-E of Balakrishnan et al. 'Genomic and physiological changes in a sexually selected and frugivorous bird radiation'.
...
## Fig.4B aBSREL
```{r, fig.width = 3, fig.height = 4, message = FALSE, warning=FALSE}
...
p_lct_anc = ggplot(df, aes(x = Enzyme, y = mean)) +
  geom_col(aes(fill = Ligand2), alpha = 0.85) +
  geom_errorbar(aes(ymin = mean - sem, ymax = mean + sem), width = 0, size = 0.2) +
  geom_text(data = tmp_stat_df, aes(y = (mean + sem) + 15, label = lab), size = FontSize * 0.75) +
  scale_y_continuous(name = 'Enzyme activity',
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        strip.background = element_blank(),
        legend.position = "none"); p_lct_anc
```
