...
- [Fig2C_Fig3A_EliotTreeMod_2020-11-16_modified.tre](#fig2cfig3a_eliottreemod_2020-11-16_modifiedtre)
- [Fig3A_treeDec15_T1R1.txt.newick](#fig3a_treedec15_t1r1txtnewick)
...
#### Positive selection JSON outputs
...
- [FigS3E_stats_sugar.csv](#figs3e_stats_sugarcsv)
...
- Format(s): .csv
- Variables:
  * species: Species or chimera pair compared
...
- Description: Newick-format species tree showing the logic of species selection for the taste receptor functional assay, used in Fig. 3C.
- Format(s): .newick
...
- Format(s): .csv
- Variables:
  * sheet: Assay plate/batch identifier
...
#### Fig3C-E_stats.csv
- Description: One-sided t-test statistics comparing taste receptor responses for sucrose against mock control used in Fig. 3C-E.
- Format(s): .csv
- Variables:
...
- Format(s): .csv
- Variables: same as Fig3B.csv (ori, Chimera, Category, Ligand, Concentraction (mM), mean, n, sd, sem)
- Missing data codes: No missing data
...
- Format(s): .csv
- Variables: same as Fig3B.csv (Ligand_ori, ori, sheet, Category, Ligand, mean, n, sd, sem)
- Missing data codes: No missing data
...
- Format(s): .csv
- Variables: same as Fig3B_stats.csv (Panel, ori, chimera, ligand_1, ligand_2, n, means, SDs, SEMs, t-statistic, df, stderr, pval, significance flags)
- Missing data codes: No missing data
...
- Description: Trimmed HyPhy aBSREL JSON output for LPH, used for visualization in Fig. 4B.
- Format(s): .json (HyPhy output)
...
- Description: SLURM batch script for building the STAR genome index from the *Manacus vitellinus* reference genome (GCF_001715985.3) and annotation. Run once before alignment.
...
- Variables (per data row):
  * Column 1: Gene ID
  * Column 2: Unstranded read count (used)
  * Column 3: Forward-strand read count
...
- Variables:
  ...
  * d: Social bonding score (ordinal: 1 = solitary, 2 = pair bond, 3 = group/colonial)
- Missing data codes: No missing data
...
- Description: Evolutionary optima estimates for plumage ornamentation, frugivory, social bonding, and ancestral state reconstruction support scores for selected focal species. Used in Fig. 5B and Fig. S5D dot plots.
- Format(s): .csv
- Variables:
  * species: Species name (Genus_species format)
  * male.color: Male plumage color/ornamentation optima estimate
  * female.color: Female plumage color/ornamentation optima estimate
  * fruit: Frugivory optima estimate
  * biparental.care: Biparental care score
  * Support: Ancestral state reconstruction support score (0-100)
